...
<?php
    ...
    function genKillLog($gdb){
        $query = $gdb->prepare("SELECT * FROM logaction WHERE action = '\"killed\"' ORDER BY time DESC LIMIT 700");
        $query->execute();
        ...
        if (count($result) != 0){
            foreach ($result as $log) {
                ...
                preg_match("/\"(.+) (killed|wounded|incapacitated) (.+) with ([^.]*)\. .+ was at \[[\d.\-,]+\] \((\d+)\)\.(?: .+ was at \[[\d.\-,]+\] \((\d+)\))?/", $str, $matches);

                if ($matches[4] == "themselves" && count($matches) != 6){
                    $victim = "";
                    $victimLocation = "";
                } else {
                    $victim = $matches[3];
                    $victimLocation = $matches[6];
                }
